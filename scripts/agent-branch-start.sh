...
build_auto_transfer_stash_argv() {
  # Emit a NUL-separated argv list: --include-untracked --message <msg> [-- :/ :(exclude,glob)PAT ...]
  local msg="$1"
  ...
  printf '%s\0' '--' ':/'
  # Read colon-separated globs into an array WITHOUT shell pathname expansion.
  local -a patterns=()
    ...
    if [[ "$AUTO_TRANSFER_ENABLED" -eq 0 ]]; then
      ...
      if git -C "$repo_root" stash push "${stash_argv[@]}" >/dev/null 2>&1; then
        ...
        if [[ -n "$auto_transfer_stash_ref" ]]; then
          ...
          # Stash succeeded but produced no entry — all dirty paths were in the exclude list.
          if has_local_changes "$repo_root"; then
